# Pokemon
🧬 <b>Pokémon Stats Analysis with Seaborn</b><br>
This project performs an exploratory data analysis (EDA) on a Pokémon dataset using Seaborn and Matplotlib. It visualizes various aspects of Pokémon attributes such as Type, Attack, Defense, Speed, and more to uncover interesting patterns and relationships.<br>
Also contains a .html file
<br><br>
## 📊 <b>Key Highlights</b>
📌 Histogram of Defense by Primary Type<br>
📌 Boxplots for Stat Distribution Across Types<br>
📌 Scatter Plots exploring stat relationships like Attack vs Defense<br>
📌 Type-Based Comparisons: Visual differences in stats across Pokémon types<br>
📌 Use of Seaborn Themes for better aesthetics<br>


## 🧰 <b>Tools / Platforms Required</b>
<li>Python 3.7+</li>
<li>Jupyter Notebook (or JupyterLab)</li>
<li>Google Colab (✅recommended for browser-based usage — no setup required)</li>
<br>

## 🧠 <b>Insights Extracted</b>
Type-based differences in Speed stat<br>
Distribution patterns show stat variability<br>
Inter-stat correlations help analyze offensive/defensive tendencies<br>
<br>

## 📄 <b>EDA Report</b><br>
<li>This project also leverages ydata-profiling to generate a comprehensive EDA report.</li>
<li>A full profiling report of the Pokémon dataset is created using <b>ydata-profiling</b>.</li>
<li>The report is exported as an .html file, which can be opened locally or uploaded to the web for sharing.</li><br>
<a href='https://steam-bell-92.github.io/Pokemon/Pokemon_Report.html'>EDA Report</a>

## 📂 <b>Dataset Source</b>
You can find the Pokémon dataset at:
<a href='https://www.kaggle.com/datasets/abcsds/pokemon'>Kaggle: Pokémon with stats<a>

## 🔎 Also you can find dataset <a href='https://github.com/steam-bell-92/Pokemon/blob/main/W1_Pokemon.csv'>(W1_Pokemon.csv)</a> above along with:
  <li>Google Colab: <a href='https://github.com/steam-bell-92/Pokemon/blob/main/Pokemon_cleaned.ipynb'>(Pokemon_cleaned.ipynb)</a></li>
  <li>HTML EDA Report (only for raw code, the website is mention above named "EDA Report"): <a href='https://github.com/steam-bell-92/Pokemon/blob/main/Pokemon_Report.html'>(Pokemon_Report.html)</a></li>
  <li><a href='https://github.com/steam-bell-92/Pokemon/blob/main/README.md'>README.md</a></li>

## 📄 License
This repository is for educational use only.
## 👤 <b>Author</b>
Anuj Kulkarni - aka - steam-bell-92

<p>Also this project was part of Summer-of-ML by BSoC</p>
